  ...
  * 22.06.2010
  */
 function PolygonCreator(map,$polygon){
 	 //create pen to draw on map
	 this.map = map;
	 this.$polygon=$polygon
	 this.pen = new Pen(this.map,this.$polygon);
	 var thisOjb=this;
 ...
 function Pen(map,$polygon){
	...
	this.draw = function(latLng){
		if (null != this.polygon) {
			console.log('Elimine el poligono anterior para crear uno nuevo');
		}
		else{
	...
	this.getData = function(){
		if(this.polygon!=null){
			...
			paths.getAt(0).forEach(function(value, index){
				data[index]=value.lat()+" "+value.lng();
				indexAll=index;
			});
			if(data[indexAll]!=data[0]) data[indexAll+1]=data[0]; // <<< -- ACA
			$polygon.val(data.join(','));
			return data;				
		}else {
			return null;
		}
