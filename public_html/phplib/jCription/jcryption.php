...
class jCryption {
	...
	*/
	private function _bitLen($num) {
		...
		$bit_len = mb_strlen($tmp) * 8;
		$tmp = ord($tmp {mb_strlen($tmp) - 1} );
		if (!$tmp) {
			$bit_len -= 8;
		} else {
			while (!($tmp & 0x80)) {
	...
	*/
	private function _redundacyCheck($string) {
		$r1 = mb_substr($string, 0, 2);
		$r2 = mb_substr($string, 2);
	...
	*/
	public function dec2string($decimal, $base) {
		...
		$charset = mb_substr($charset, 0, $base);

		do {
			$remainder = bcmod($decimal, $base);
			$char = mb_substr($charset, $remainder, 1);
			$string = $char . $string;
...
class AesCtr extends Aes {
  ...
  public static function encrypt($plaintext, $password, $nBits) {
    ...
    $pwBytes = array();
    for ($i=0; $i<$nBytes; $i++) $pwBytes[$i] = ord(mb_substr($password,$i,1)) & 0xff;
    ...
    $blockCount = ceil(mb_strlen($plaintext)/$blockSize);
    ...
    for ($b=0; $b<$blockCount; $b++) {
      ...
      $blockLength = $b<$blockCount-1 ? $blockSize : (mb_strlen($plaintext)-1)%$blockSize+1;
      $cipherByte = array();
      
      for ($i=0; $i<$blockLength; $i++) {  // -- xor plaintext with ciphered counter byte-by-byte --
        $cipherByte[$i] = $cipherCntr[$i] ^ ord(mb_substr($plaintext, $b*$blockSize+$i, 1));
  ...
  public static function decrypt($ciphertext, $password, $nBits) {
    ...
    $counterBlock = array();
    $ctrTxt = mb_substr($ciphertext, 0, 8);
    for ($i=0; $i<8; $i++) $counterBlock[$i] = ord(mb_substr($ctrTxt,$i,1));
    
    // generate key schedule
    $keySchedule = Aes::keyExpansion($key);
  
    // separate ciphertext into blocks (skipping past initial 8 bytes)
    $nBlocks = ceil((mb_strlen($ciphertext)-8) / $blockSize);
    $ct = array();
    for ($b=0; $b<$nBlocks; $b++) $ct[$b] = mb_substr($ciphertext, 8+$b*$blockSize, 16);
    ...
    for ($b=0; $b<$nBlocks; $b++) {
      ...
      $plaintxtByte = array();
      for ($i=0; $i<mb_strlen($ciphertext[$b]); $i++) {
        // -- xor plaintext with ciphered counter byte-by-byte --
        $plaintxtByte[$i] = $cipherCntr[$i] ^ ord(mb_substr($ciphertext[$b],$i,1));
