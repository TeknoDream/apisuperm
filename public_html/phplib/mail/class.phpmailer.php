...
class PHPMailer {
  ...
  function WrapText($message, $length, $qp_mode = false) {
    ...
    $is_utf8 = (mb_strtolower($this->CharSet) == "utf-8");
  ...
  function EncodeHeader ($str, $position = 'text') {
    ...
    if (mb_strlen($str)/3 < $x) {
      $encoding = 'B';
      if (function_exists('mb_mb_strlen') && $this->HasMultiBytes($str)) {
  ...
  function HasMultiBytes($str) {
    if (function_exists('mb_mb_strlen')) {
      return (mb_strlen($str) > mb_mb_strlen($str, $this->CharSet));
  ...
  function Base64EncodeWrapMB($str) {
    ...
    $mb_length = mb_mb_strlen($str, $this->CharSet);
    ...
    for ($i = 0; $i < $mb_length; $i += $offset) {
      ...
      do {
        ...
        $chunk = mb_mb_substr($str, $i, $offset, $this->CharSet);
    ...
    $encoded = mb_substr($encoded, 0, -mb_strlen($this->LE));
    return $encoded;
  }
    ...
    if(isset($images[2])) {
      foreach($images[2] as $i => $url) {
        ...
        if (!preg_match('/^[A-z][A-z]*:\/\//',$url)) {
          ...
          if ( mb_strlen($directory) > 1 && mb_substr($basedir,-1) != '/') { $directory .= '/'; }
  ...
  function _mime_types($ext = '') {
    ...
    return ( ! isset($mimes[mb_strtolower($ext)])) ? 'application/octet-stream' : $mimes[mb_strtolower($ext)];
  }
