	...
	$.fn.tagedit = function(options) {
		...
		*/
		options = $.extend(true, {
			...
			texts: {
				removeLinkTitle: 'Eliminar de la lista.',
				saveEditLinkTitle: 'Guardar cambios.',
				deleteLinkTitle: 'Eliminar esta etiqueta de la base de datos.',
				deleteConfirmation: 'Está seguro de eliminar esta etiqueta?',
				deletedElementTitle: 'Este elemento será eliminado.',
				breakEditLinkTitle: 'Cancelar',
                forceDeleteConfirmation: 'Hay mas elementos usando este tag, está seguro de eliminarlo?'
			}
		}, options || {});
		...
		*/
		function inputsToList() {
			if(options.tabindex!=undefined)
				var html = '<ul class="tagedit-list '+options.additionalListClass+'" tabindex="'+options.tabindex+'">';
			else
			...
			elements.each(function() {
				...
				if(element_name && element_name.length == 4 && (options.deleteEmptyItems == false || $(this).val().length > 0)) {
					if(element_name[1].length > 0) {
						...
						html += '<li class="tagedit-listelement tagedit-listelement-old '+options.additionalListClassLi+'">';
			...
			html = '<li class="tagedit-listelement tagedit-listelement-new">'; /*disabled="disabled"*/
			html += '<input type="text" name="'+baseName+'[]" value="" id="tagedit-input" disabled="disabled" tabindex="'+options.tabindex+'" class="tagedit-input-disabled" dir="'+options.direction+'"/>';
			html += '</li>';
			html += '</ul>';
						...
						$(this).bind('transformToTag', function(event, id) {
							...
							if(isNewResult[0] === true || (isNewResult[0] === false && typeof isNewResult[1] == 'string')) {
								...
								if(options.allowAdd == true || oldValue) {
									// Añadido por Chelo Rueda
									var TextAdd=$(this).val()
									,	TextAddArray=TextAdd.split(',')
									for (var kI in TextAddArray){

										// Make a new tag in front the input
										html = '<li class="tagedit-listelement tagedit-listelement-old '+options.additionalListClassLi+'">';
										html += '<span dir="'+options.direction+'">' + TextAddArray[kI].trim() + '</span>';
										var name = oldValue? baseName + '['+id+options.addedPostfix+']' : baseName + '[]';
										html += '<input type="hidden" name="'+name+'" value="'+TextAddArray[kI].trim()+'" />';
										html += '<a class="tagedit-close" title="'+options.texts.removeLinkTitle+'">x</a>';
										html += '</li>';
